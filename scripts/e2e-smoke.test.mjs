...
const MODEL = process.env.PI_E2E_SMOKE_MODEL ?? "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free";
...
function runCli(args, { cwd, env, timeoutMs }) {
	return new Promise((resolveRun, reject) => {
		const child = spawn(TSX, args, { cwd, env, stdio: ["ignore", "pipe", "pipe"] });
		let stdout = "";
		let stderr = "";
...
test(
	...
	{ skip: process.env.OPENROUTER_API_KEY ? false : "OPENROUTER_API_KEY not set; skipping e2e smoke", retry: 1 },
	async () => {
		...
		try {
			...
			const { code, stdout, stderr } = await runCli(
				["--tsconfig", join(REPO_ROOT, "tsconfig.json"), CLI, "-p", "--model", MODEL, PROMPT],
			...
			assert.match(stdout, /hello from smoke/, `model output missing the file contents (${details})`);
		} finally {
