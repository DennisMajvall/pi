...
const allowValue = "1"; // process.env.PI_ALLOW_LOCKFILE_CHANGE;
const allowed =
  allowValue === "1" || allowValue === "true" || allowValue === "yes";
...
function hasOnlyWorkspacePackageChanges(changes) {
  return (
    changes.length > 0 &&
    changes.every((change) => isWorkspacePackagePath(change.lockPath))
  );
}

function summarizeLockfileChange(changes) {
  const nodeModuleChanges = changes.filter((change) =>
    change.lockPath.includes("node_modules/"),
  );
  const summary = [];
...
if (allowed) {
  console.error(
    "package-lock.json is staged; PI_ALLOW_LOCKFILE_CHANGE is set, allowing commit.",
  );
  process.exit(0);
}
...
if (changes && hasOnlyWorkspacePackageChanges(changes)) {
  console.error(
    "package-lock.json only updates workspace package metadata; allowing commit.",
  );
  process.exit(0);
}
...
console.error(
  "  - regenerate/check coding-agent shrinkwrap if release deps changed",
);
