...
- Fixed the `/plans` detail view truncating plan content into a narrow pane; the whole plan now renders as a word-wrapped document across the full terminal width (goal, strategy, constraints, assumptions, tasks, DAG, and revisions), readable via terminal scrollback, with themed colors for headers, labels, ids, and status tags plus a blank line under each section heading.
- Fixed `/plans` edits to fall back to the session model when the deterministic edit parser does not recognize a free-form directive, applying the change to the plan and recording a new `user_edit` revision; an animated status line shows while an edit is being applied.
