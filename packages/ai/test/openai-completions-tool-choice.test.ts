...
describe("openai-completions tool_choice", () => {
  ...
  it("preserves z.ai thinking when replaying reasoning_content", async () => {
    ...
    const replayedAssistant = params.messages?.find(
    ...
    mockState.chunks = [
      {
        ...
        choices: [
          { delta: { content: "complete answer" }, finish_reason: null },
        ],
      },
    ];
  ...
  it("accumulates mixed content, reasoning, and parallel tool call deltas independently", async () => {
    ...
    expect(eventTypes.filter((type) => type === "thinking_start")).toHaveLength(
      1,
    );
    expect(eventTypes.filter((type) => type === "thinking_delta")).toHaveLength(
      2,
    );
    expect(eventTypes.filter((type) => type === "thinking_end")).toHaveLength(
      1,
    );
    expect(eventTypes.filter((type) => type === "toolcall_start")).toHaveLength(
      4,
    );
    expect(eventTypes.filter((type) => type === "toolcall_delta")).toHaveLength(
      9,
    );
  ...
  it("stores OpenRouter Kimi K2.6 reasoning replay compat in built-in metadata", () => {
    ...
    expect(model.compat?.requiresReasoningContentOnAssistantMessages).toBe(
      true,
    );
  });
  ...
  it("normalizes OpenCode Go reasoning deltas to reasoning_content for replay", async () => {
    ...
    const response = await streamSimple(
      ...
      {
        messages: [
          { role: "user", content: "Use reasoning.", timestamp: Date.now() },
        ],
      },
      { apiKey: "test" },
    ).result();
  ...
  it("omits disabled thinking for Moonshot Kimi K2.7 Code models", async () => {
    const cases = [
      ...
      getModel("moonshotai-cn", "kimi-k2.7-code"),
    ];
  ...
  it("sends max_tokens for OpenCode completions models", async () => {
    const cases = [
      getModel("opencode-go", "kimi-k2.6")!,
  ...
  it("omits Ant Ling reasoning for unmapped direct reasoning efforts and non-reasoning models", async () => {
    ...
    expect(
      (payload ?? mockState.lastParams) as { reasoning?: unknown },
